...
export function findEarliestDate(shifts: TeamsShiftRow[]): Date {
  ...
  for (const shift of shifts) {
    const [month, day, year] = shift.startDate.split('/').map(n => parseInt(n));
 ...
 */
export function getDayOfWeekFromMonday(dateStr: string, mondayDate: Date): string {
 ...
 */
export function aggregateSchedules(shifts: TeamsShiftRow[]): ProcessedSchedule[] {
  if (shifts.length === 0) {
    return [];
  }
  ...
  for (const shift of shifts) {
    ...
    try {
      ...
      console.warn(`Skipping invalid time block for ${shift.name}: ${timeBlock}`, error);
    }
  }
  
  // Sort time blocks for each day
  for (const student of studentMap.values()) {
    for (const day of Object.keys(student.availability) as Array<keyof typeof student.availability>) {
