...
export default function PublicClockSystem() {
      ...
      prev.map((staff) => {
        if (staff.id === staffId) {
          ...
          const updatedStaff: any = {
            ...
            todayActual:
              type === 'in' ? formatTime(new Date()) : staff.todayActual,
          };
  ...
  const handleCardSwipe = (cardData: string) => {
    ...
    if (staff) {
      ...
      setClockInMessage(
        `${staff.name} clocked ${action} at ${formatTime(new Date())}`
      );
  ...
  const handleManualClockIn = (staffId: number, isManual: boolean) => {
    ...
    if (staff) {
      ...
      setClockInMessage(
        `${staff.name} clocked ${action} at ${formatTime(new Date())}${manualFlag}`
      );
      setShowClockInSuccess(true);
  ...
  const handleManualClockOut = (staffId: number, isManual: boolean) => {
    ...
    if (staff) {
      ...
      const manualFlag = isManual ? ' (Manual Entry)' : '';
      setClockInMessage(
        `${staff.name} clocked out at ${formatTime(new Date())}${manualFlag}`
      );
      setShowClockInSuccess(true);
